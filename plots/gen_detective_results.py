...
com média±desvio), mas para o cenário de ATAQUE (não falha):
  - t=0 alinhado ao início do ataque (parseado de ATTACK_CONFIG, default 300s)
  - QI/SR caem no impacto e (em v1/mode1) recuperam via pipeline de órfãos

Saídas:
  plots/data/summary_runs.csv          — 1 linha por run (métricas-chave)
  plots/figures/fig_qi_attack.{pdf,png}        — QI alinhado ao ataque, média±std por escala
  plots/figures/fig_sr_attack.{pdf,png}        — SR alinhado ao ataque
  plots/figures/fig_attacker_learning.{pdf,png}— Q-atacante (QUARANTINE) + epsilon vs run
  plots/figures/fig_recovery_summary.{pdf,png} — SR pré/vale/final por escala (barras)

Uso:
  python3 plots/gen_detective_results.py                 # auto-detecta *_attack* em results-detective/
  python3 plots/gen_detective_results.py --glob '*_attack5'
"""
import os
import re
import glob
...
def load_scenario(scen_dir):
    """Devolve (df_runlevel, df_aligned) para uma escala."""
    ...
    for i, rd in enumerate(run_dirs):
        ...
        run_rows.append({
            ...
            'atk_qquar_high': float(df['qAtkQuarHigh'].iloc[-1]),
            'atk_qquar_low': float(df['qAtkQuarLow'].iloc[-1]),
...
def main():
    ...
    # Tabela-resumo por escala (impressa + salva)
    print('\n=== RESUMO POR ESCALA ===')
    rows = []
    for s in sorted(summary['scale'].unique()):
        g = summary[summary.scale == s]
        total_quar = g.atk_quar_total.sum()
        total_att = (g.n_attackers).sum()
        rows.append({
            'scale': s, 'runs': len(g),
