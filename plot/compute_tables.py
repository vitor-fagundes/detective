...
RESULTS_QL = os.path.join(PROJ, 'results-qlearning')
...
def analyze(scen, N):
    ...
    A = {k: [] for k in ['leaders','sat','orphans','realloc','recluster','srpre','srmin']}
    adt, amt = [], []
    ...
    for rd in sorted(glob.glob(os.path.join(scen, 'run_*'))):
        ...
        A['recluster'].append(float(df['actionsRecluster'].sum()))
...
for scen, N in SERIES:
    ...
    print(f'    Leaders={ms(A["leaders"])}  Sat={ms(A["sat"])}  Orphans={ms(A["orphans"])} '
          f'Realloc={ms(A["realloc"])}  Recluster={ms(A["recluster"])}  SRpre={ms(A["srpre"])}  SRmin={ms(A["srmin"])}')
...
# Baseline QL: só a TABELA 1 (rede) faz sentido — sem detecção/quarentena,
# TABELA 2 (segurança) seria degenerada (TP=FP=0 sempre, ver processAttackersIntuitively
# desabilitado no modo qlearning).
d = os.path.join(RESULTS_QL, '250_attack5')
if os.path.isdir(d):
    A, adt, amt, (ta, tb, tc), C = analyze(d, 250)
    print(f'\n===== QL(250)  (N=250, {C["nruns"]} runs) =====')
    print('  TABELA 1 (rede) [mean ± 95% CI]:')
    print(f'    Leaders={ms(A["leaders"])}  Sat={ms(A["sat"])}  Orphans={ms(A["orphans"])} '
          f'Realloc={ms(A["realloc"])}  Recluster={ms(A["recluster"])}  SRpre={ms(A["srpre"])}  SRmin={ms(A["srmin"])}')
    print('  TABELA 2 (segurança): N/A — modo qlearning não tem detecção/quarentena')
